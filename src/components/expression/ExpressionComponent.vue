...
const getSavedSize = () => {
  const savedSize = localStorage.getItem('expression-editor-splitter-position')
  if (savedSize) {
...
const blocklyComponentRef = ref<InstanceType<typeof BlocklyExpressionComponent> | null>(null)
const monacoEditorRef = ref<InstanceType<typeof MonacoExpressionEditor> | null>(null)
const splitpanesReady = ref(false) // 标记 splitpanes 是否已准备好

// 计算右侧面板大小
const rightPaneSize = computed(() => 100 - leftPaneSize.value)

// 处理 Blockly 块选择事件 (简化版本 - 仅记录，不做高亮)
const handleBlocklyBlockSelect = (blockInfo: { blockId: string, expression: string, startPos: number, endPos: number }) => {
  console.debug('ExpressionComponent received block-select:', blockInfo)
  // 简化：仅记录事件，不进行双向高亮
}

// 处理 Monaco 选择事件 (简化版本 - 仅记录，不做高亮)
const handleMonacoSelectionChange = (selectionInfo: { startPos: number, endPos: number }) => {
  console.debug('️ Monaco selection changed:', selectionInfo)
  console.debug(`Selected text: "${props.modelValue.substring(selectionInfo.startPos, selectionInfo.endPos)}"`)
  // 简化：仅记录事件，不进行双向高亮
}
...
const handleResize = throttle((event: any) => {
  ...
  if (event && event.prevPane && typeof event.prevPane.size === 'number') {
    ...
    localStorage.setItem('expression-editor-splitter-position', newSize.toString())
  }
...
const resetLayout = () => {
  ...
  localStorage.setItem('expression-editor-splitter-position', '50')
  nextTick(() => {
    // 模拟正确的事件格式来触发编辑器重新布局
  ...
  <div class="container">
    <div class="toolbar">
      ...
      <div class="toolbar-info">
        <span class="info-text">数学表达式编辑器 (支持 MathJS 语法)</span>
      </div>
    </div>
    <Splitpanes class="default-theme main-splitpanes" @resize="handleResize" @ready="onSplitpanesReady"
      ...
      <Pane :size="leftPaneSize" :min-size="20" :max-size="80" class="blockly-pane">
        <BlocklyExpressionComponent ref="blocklyComponentRef" v-model="code" :parent-ready="splitpanesReady"
          @block-select="handleBlocklyBlockSelect" />
      </Pane>
      <Pane :size="rightPaneSize" class="code-pane">
        <MonacoExpressionEditor ref="monacoEditorRef" v-model="code" />
      </Pane>
    </Splitpanes>
  </div>
</template>
